...
<div class="container-fluid py-4">
    ...
    <div class="row">
        <div class="col-lg-8">
            ...
            @if($booking->booking_status === 'confirmed' && !$booking->hidden_at)
                <x-card title="Room Assignment" icon="fas fa-door-open" bodyClass="card-body" class="mb-4">
                    <div class="alert alert-danger d-none" id="assignRoomsInlineError"></div>
                        ...
                        <ul class="list-unstyled mb-3">
                            @foreach($booking->rooms as $room)
                                <li class="mb-1">
                                    ...
                                    &middot; Sleeps {{ $room->room_capacity }}
                                    &middot; &#8369;{{ number_format($room->room_rate, 2) }}/night
                                </li>
                            @endforeach
                        </ul>
                        @if($booking->rooms->count() < $booking->rooms_requested)
                            <p class="text-warning small mb-3"><i class="fas fa-exclamation-triangle"></i> {{ $booking->rooms_requested - $booking->rooms->count() }} more room(s) still need to be assigned.</p>
                        @endif
                    @else
                        <p class="text-muted mb-3">No room assigned yet.</p>
                    @endif
                    <button type="button" class="btn btn-primary btn-open-assign-rooms" data-booking-id="{{ $booking->id }}">
